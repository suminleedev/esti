...
class VendorBUrinalSinkSheetTest {
    ...
    void 소변기_U135는_스퍼드_후렌지_부속과_세트가95500() {
        ...
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(u135.setPrice()), "計=95500");
        // 도기 80000 + 스퍼드 3500 + 후렌지 12000 = 95500
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(part(u135, "스퍼드").unitPrice()));
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(part(u135, "후렌지").unitPrice()));
    ...
    void req2_수채_SS131은_수채가랑_수채트랩부속_계122000이고_소변기슬롯_오염없음() {
        ...
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(ss131.setPrice()), "計=122000");

        // 수채 전용 부속명/단가 (소변기 헤더의 스퍼드/후렌지가 아니어야 함)
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(part(ss131, "수채가랑").unitPrice()));
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(part(ss131, "수채트랩").unitPrice()));
    ...
    void 소변기_IU302E_슬롯의_설명텍스트는_부속이아니라_description에_저장() {
        ...
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(iu302.setPrice()));
    }
    ...
    void 수채_SS132_세트가134000_품종_수채_carryforward() {
        ...
        assertEquals(0, new BigDecimal("<PRICE>").compareTo(ss132.setPrice()));
