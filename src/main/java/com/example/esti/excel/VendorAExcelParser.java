...
public class VendorAExcelParser implements VendorExcelParser {
    ...
    // =====================================================================================
    // P1: 새 출력 구조(VendorProductSet) — 대표품목 + 부속 묶음
    //  - A·B열 제외(D11): C(2)=소분류/세트명, D(3)=구성품명, E(4)=구품번, F(5)=신품번, G(6)=단가
    //  - 합계행(G만 있는 행)이 세트 경계이자 대표품목 가격
    //  - 그룹핑(D16): 직전 연속 부속 합이 합계와 "일치"하면 부속으로 연결,
    //                "불일치"면 대표품목(첫 행)만 합계가로 저장 + needsReview=true,
    //                나머지 행은 개별(독립) 제품으로 저장
