...
class VendorB2026ToiletSheetTest {
    ...
    void 세로_나열형_세트는_計와_구성합이_일치하고_첫행이_MAIN이다() {
        ...
        assertEquals(new BigDecimal("160500"), s.setPrice());
        ...
        assertEquals(new BigDecimal("86000"), dogi.unitPrice());
    ...
    void 투피스는_하부가_대표품목이_된다() {
        ...
        assertEquals(new BigDecimal("402700"), s.setPrice());
    ...
    void 동일_품번이_두_번_나오면_별개_세트로_갈린다() {
        ...
        assertEquals(new BigDecimal("273500"), first.setPrice());
        assertEquals(new BigDecimal("229000"), second.setPrice());
    ...
    void 세트_사이_빈_행은_세트만_끊고_시트를_끝내지_않는다() {
        ...
        List<VendorProductSet> sets = parse();
        assertEquals(new BigDecimal("417800"), byCode(sets, "IC600DE").setPrice());
        assertEquals(new BigDecimal("449800"), byCode(sets, "IC599DE").setPrice(),
