...
public class QuoteNumberService {
    ...
    public String assign(Proposal proposal) {
        ...
        int next = proposalRepository.findTopByQuoteNoStartingWithOrderByQuoteNoDesc(prefix)
                .map(p -> parseSequence(p.getQuoteNo(), prefix) + 1)
                .orElse(1);

        String quoteNo = prefix + String.format("%0" + SEQUENCE_LENGTH + "d", next);
