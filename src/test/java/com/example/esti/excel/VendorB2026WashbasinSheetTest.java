...
class VendorB2026WashbasinSheetTest {
    ...
    void 세트가는_구성합이_아니라_기본_조합가다() {
        ...
        assertEquals(new BigDecimal("101100"), s.setPrice());
        assertEquals(new BigDecimal("212500"), sumOf(s));
    ...
    void 세트_중간의_計는_다른_조합가라_세트를_끊지_않는다() {
        ...
        assertEquals(new BigDecimal("101100"), s.setPrice(), "세트가는 시작 행의 計만 쓴다");
    }
    ...
    void 도기_변형이_여러_개면_전부_구성으로_남는다() {
        ...
        assertEquals(List.of("도기(독립폽업)", "도기(103비누대)", "도기(103/102비누대)",
                "비누대(SP103)", "비누대"), names);
        assertEquals(new BigDecimal("95600"), s.setPrice(), "기본 조합 = 도기 단독");
    ...
    void 시트_하단_부속_부록표는_적재되지_않는다() {
        ...
        assertEquals(new BigDecimal("66900"), last.setPrice());
    }
    ...
    void 동일_품번_변형은_갈라진다() {
        ...
        assertEquals(new BigDecimal("131200"), byCode(sets, "IL672").setPrice());
        assertEquals(new BigDecimal("90200"), byCode(sets, "IL672-2").setPrice());
        assertEquals(new BigDecimal("82200"), byCode(sets, "IL672-3").setPrice());
    }
