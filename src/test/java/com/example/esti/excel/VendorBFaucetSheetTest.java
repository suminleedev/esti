...
class VendorBFaucetSheetTest {
    ...
    void 품번없이_제품코드만_있는_부속도_유실없이_적재() {
        // G-0113 국산: 후렉시블호스(냉/온)은 품번(C) 없이 제품코드(D=<CODE>)만 → D를 코드로, 냉/온 c/h 분리
        VendorProductSet kor = byCode(faucetByBasis(KOR), "G-0113");
        assertEquals("G-0113_<CODE>c", part(kor, "후렉시블호스(냉)").productCode());
        assertEquals("G-0113_<CODE>h", part(kor, "후렉시블호스(온)").productCode());
    }
