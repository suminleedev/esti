...
class CatalogImport2026IntegrationTest {
    ...
    void 최신본_적재_후_재업로드가_멱등이다() {
        ...
        assertThat(productRepository.findByVendorAndProductCode(b, "43u9013c")).isPresent();
        assertThat(productRepository.findByVendorAndProductCode(b, "43dbu9013c")).isPresent();
        // 수전금구(T6) — 전산코드를 보조 코드로 보존
        assertThat(productRepository.findByVendorAndProductCode(b, "G-0110")).isPresent();
        // 바스(T8) — 신규 카테고리
        assertThat(productRepository.findByVendorAndProductCode(b, "6ibc6015a").orElseThrow()
